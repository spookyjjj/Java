...
public class T0524_1 {
	public static void main (String args[]) {
		...
		*/
		...
		//4. 사용자에게 비밀번호를 물어보고 동일할 경우 통과 최대 3번의 기회를 줌
		/* ★★★선생님 답안★★★ -> do while을 쓴것, while조건안에 || && 쓴것, 탈출 후에 if else로 분류한것
		String pw = "1q2w3e";
		int pwcount = 3;
		String pwinput;
		do {
			...
			pwcount--;
			System.out.println("현재 카운트값 확인: " + pwcount);
		} while (!pwinput.equals(pw) && pwcount > 0);
		
		if (pwinput.equals(pw)) {
			System.out.println("로그인");
		} else {
			System.out.println("비밀번호 확인 후 다시 시도해 주세요");
		} */
		
		scan.nextLine(); //위에서 nextInt때문에..
		String pw = "q1w2e3";
		int pwcount = 0;
		while (pwcount < 3) {
			System.out.println("비밀번호를 입력하세요");
