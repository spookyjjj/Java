...
public class Main6 {
	...
	public static void main(String[] args) {
		...
		String concat = str.concat(other);
	
		System.out.println(concat);
		System.out.println("원본문자열다른문자열" == concat); //(false) 참조가 달라서 
		System.out.println(conorigin == concat); //(false) 원래부터 그 문자인 것과 합연산으로 하나가 된 아이는 메모리절약이고 뭐고 같은 참조 아님
	
		//그래서 합연산하면 계속 새로운 공간을 잡아먹게 되는데, 이걸 보완한 클래스가 있다 -> 합연산이 자주 일어나는 경우에는 이걸써야 효율좋음 
		//=================StringBuilder
